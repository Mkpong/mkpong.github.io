---
title: "소개"
description: "AI 인프라를 공부하는 대학원생 Jaeyoung 입니다."
showToc: false
hidemeta: true
comments: false
ShowBreadCrumbs: false
---

안녕하세요, **Jaeyoung** 입니다. AI 인프라 분야를 공부하는 대학원생이고, 이 블로그에는 공부하면서 배운 것과 삽질한 기록을 한국어로 남깁니다.

큰 모델을 학습하고 서빙하는 일은 결국 **GPU 몇 장을 얼마나 효율적으로 굴리느냐**의 문제라고 생각합니다. 그래서 프레임워크보다 그 아래 — 스케줄러, 네트워크, 메모리, 커널 — 에 더 관심이 많습니다.

## 관심 분야

- **분산 학습** — 데이터/텐서/파이프라인 병렬, NCCL, 통신-연산 오버랩
- **모델 서빙** — vLLM, Triton, KV 캐시 관리, 배칭 전략
- **GPU 스케줄링** — Kubernetes 위의 GPU 리소스 관리, MIG/MPS, 큐잉
- **성능 분석** — 프로파일링, 병목 찾기, 벤치마크 설계
- **MLOps** — 실험 관리, 재현 가능한 학습 파이프라인

## 이력

<!-- 아래 항목은 자리표시자입니다. 실제 내용으로 바꿔 주세요. -->

{{< timeline >}}
{{< tl-item date="2026.03 – 현재" title="OO대학교 대학원 석사과정" sub="OO학과 · OO 연구실" >}}
AI 인프라 / 시스템 분야 연구
{{< /tl-item >}}
{{< tl-item date="2022.03 – 2026.02" title="OO대학교 학사" sub="OO학과" >}}
{{< /tl-item >}}
{{< tl-item date="2025.07 – 2025.08" title="OO 인턴" sub="ML 플랫폼팀" >}}
- 학습 클러스터 운영 자동화
- 모델 서빙 파이프라인 개선
{{< /tl-item >}}
{{< /timeline >}}

## 기술 스택

| 영역 | 주로 쓰는 것 |
|---|---|
| 언어 | Python, C++, Go, Bash |
| ML | PyTorch, CUDA, NCCL, Triton |
| 인프라 | Kubernetes, Docker, Slurm, Terraform |
| 관측 | Prometheus, Grafana, Nsight Systems |

## 이 블로그에 대해

- [Hugo](https://gohugo.io/) 와 [PaperMod](https://github.com/adityatelange/hugo-PaperMod) 테마로 만들었고, GitHub Actions 로 GitHub Pages 에 배포합니다.
- 글에 오류가 있거나 더 나은 방법이 있다면 각 글 상단의 **수정 제안** 링크나 GitHub 이슈로 알려 주세요.
- 소스: [github.com/mkpong/mkpong.github.io](https://github.com/mkpong/mkpong.github.io)

## 연락

- GitHub: [@mkpong](https://github.com/mkpong)
- Email: [leeja042499@gmail.com](mailto:leeja042499@gmail.com)
